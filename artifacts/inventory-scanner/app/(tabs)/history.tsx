    ...
    .map(
      (h) =>
        `<tr>
          ...
          <td style="text-align:right">${(h.unitPrice ?? 0).toFixed(2)}</td>
          <td style="text-align:right">${(h.amount ?? 0).toFixed(2)}</td>
        </tr>`,
    )
    .join("");
...
<style>
  ...
  .badge{display:inline-block;padding:3px 10px;border-radius:12px;font-size:12px;font-weight:700;margin-bottom:16px}
  .sale{background:#fee2e2;color:#b91c1c}
  .purchase{background:#dcfce7;color:#15803d}
  .credit{background:#fef3c7;color:#b45309}
  ...
  .total-row td{font-weight:700;font-size:14px;border-top:2px solid #111;border-bottom:none;padding-top:12px}
  .person{font-size:14px;font-weight:700;color:#d97706;margin-bottom:8px}
...
<body>
  ...
  <div style="text-align:center">
    <span class="badge ${bill.type.toLowerCase()}">${typeLabel}</span>
  </div>
  ${bill.personName ? `<p class="person">Customer: ${bill.personName}</p>` : ""}
  <table>
    <thead>
      <tr>
        <th>Item</th>
  ...
  <p class="footer">Printed from Inventory Scanner</p>
</body>
</html>`;
}
...
export default function HistoryScreen() {
  ...
  const handleReturnBill = (bill: BillGroup) => {
    ...
    if (!returnable.length) { Alert.alert("", t("alreadyReturned")); return; }
  ...
  const renderBill = ({ item }: { item: BillGroup }) => {
    ...
    const allReturned =
      (isSale || isCredit) && item.items.every((h) => h.returned);
      ...
      <View style={[styles.billCard, { backgroundColor: colors.card }]}>
        ...
        >
          ...
          <View style={styles.billInfo}>
            {isCredit && item.personName ? (
              <Text style={[styles.billPerson, { color: colors.warning }, rtl && styles.rtlText]} numberOfLines={1}>
                {item.personName}
              </Text>
            ) : null}
            {isReturn && (
              <Text style={[styles.billPerson, { color: "#0ea5e9" }, rtl && styles.rtlText]}>
                {t("returnType")}
              </Text>
            )}
            {isMulti ? (
              <Text style={[styles.billName, { color: colors.foreground }, rtl && styles.rtlText]}>
                {t("billItems", item.items.length)}
              </Text>
            ) : (
              <Text style={[styles.billName, { color: colors.foreground }, rtl && styles.rtlText]} numberOfLines={1}>
                {item.items[0]?.name ?? ""}
              </Text>
            )}
            <Text style={[styles.billDate, { color: colors.mutedForeground }, rtl && styles.rtlText]}>
              {fmtDate(item.date)}
            </Text>
          </View>
          <View style={styles.billRight}>
            ...
            {isCredit && (
              <View style={[styles.badge, { backgroundColor: item.paid ? "#dcfce7" : "#fef3c7" }]}>
                <Text style={{ color: item.paid ? "#166534" : "#b45309", fontSize: 8, fontWeight: "800" }}>
            ...
            {allReturned && (
              <View style={[styles.badge, { backgroundColor: "#e0f2fe" }]}>
                <Text style={{ color: "#0369a1", fontSize: 8, fontWeight: "800" }}>{t("returnBadge")}</Text>
              </View>
            )}
            {isMulti && (
        ...
        <View style={[styles.actionRow, { borderTopColor: colors.border }, rtl && styles.rowReverse]}>
          ...
          <TouchableOpacity
            ...
            disabled={isPrinting}
          >
            <Feather name="printer" size={12} color="#7c3aed" />
            <Text style={[styles.actionBtnText, { color: "#7c3aed" }]}>
              {isPrinting ? "..." : t("printBill")}
            </Text>
          </TouchableOpacity>
        </View>

        {isOpen && (
          <View style={[styles.billItems, { borderTopColor: colors.border }]}>
            {item.items.map((h) => {
              const canReturnLine = !h.returned && (h.type === "SALE" || h.type === "CREDIT");
              return (
                <View key={h.id} style={[styles.lineRow, rtl && styles.rowReverse]}>
                  <View style={{ flex: 1 }}>
                    <View style={{ flexDirection: rtl ? "row-reverse" : "row", alignItems: "center", gap: 6 }}>
                      <Text
                        style={[styles.lineName, { color: h.returned ? colors.mutedForeground : colors.foreground }, rtl && styles.rtlText]}
                        numberOfLines={1}
                      >
                        {h.name}
                      </Text>
                    ...
                    {(h.unitPrice ?? 0) > 0 && (
                      <Text style={[styles.lineMeta, { color: colors.mutedForeground }, rtl && styles.rtlText]}>
                  ...
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                    {canReturnLine && (
                      <TouchableOpacity onPress={() => handleReturnEntry(h.id, h.name)} style={styles.lineReturnBtn}>
                        <Feather name="rotate-ccw" size={11} color="#0ea5e9" />
                      </TouchableOpacity>
                    )}
                    <Text style={[styles.lineAmount, { color: typeColor }]}>
                      {sign}{h.qty}{(h.amount ?? 0) > 0 ? `  ${(h.amount ?? 0).toFixed(2)}` : ""}
                    </Text>
                  </View>
                </View>
              );
    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={[styles.header, { paddingTop: headerTopPadding, backgroundColor: colors.card, borderColor: colors.border }]}>
        ...
        <View style={[styles.summary, rtl && styles.rowReverse]}>
          ...
          <TouchableOpacity style={[styles.csvBtn, { backgroundColor: colors.primary }]} onPress={exportCSV}>
      ...
      <View style={[styles.tabs, { backgroundColor: colors.card, borderColor: colors.border }, rtl && styles.rowReverse]}>
        {(["ALL", "SALE", "PURCHASE", "CREDIT", "RETURN"] as Filter[]).map((key) => {
          const label = key === "ALL" ? t("all") : key === "SALE" ? t("sale") : key === "PURCHASE" ? t("purchase") : key === "CREDIT" ? t("credit") : t("returnType");
          const active = filter === key;
          return (
            <TouchableOpacity
              key={key}
              style={[styles.tab, { backgroundColor: active ? colors.primary : colors.secondary }]}
              onPress={() => setFilter(key)}
            >
              <Text style={[styles.tabText, { color: active ? "white" : colors.mutedForeground }]}>{label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
      ...
      <View style={[styles.searchWrap, { backgroundColor: colors.card, borderColor: colors.border }, rtl && styles.rowReverse]}>
        ...
        <TextInput
          style={[styles.searchInput, { color: colors.foreground }, rtl && styles.rtlInput]}
      ...
      {filteredBills.length === 0 ? (
        <View style={styles.empty}>
          ...
          <Text style={[styles.emptyText, { color: colors.mutedForeground }, rtl && styles.rtlText]}>{t("noHistory")}</Text>
        </View>
      ) : (
        <FlatList
          data={filteredBills}
      ...
      {history.length > 0 && (
        <TouchableOpacity
          style={[styles.clearBtn, { backgroundColor: "#fef2f2", borderColor: "#fca5a5", marginBottom: insets.bottom + 80 }]}
          onPress={handleClear}
        >
          <Feather name="trash-2" size={14} color={colors.destructive} />
          <Text style={[styles.clearText, { color: colors.destructive }, rtl && styles.rtlText]}>{t("clearHistory")}</Text>
        </TouchableOpacity>
      )}
    </View>
  );
...
function SumItem({ num, label, color, mutedColor }: { num: string; label: string; color: string; mutedColor: string }) {
  return (
    <View style={{ flex: 1, alignItems: "center" }}>
      ...
      <Text style={{ fontSize: 8, color: mutedColor, textTransform: "uppercase", letterSpacing: 0.4, fontWeight: "600" }}>{label}</Text>
    </View>
  );
}
